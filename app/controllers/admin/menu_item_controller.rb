class Admin::MenuItemController < Managed
  ...
  def controller_actions
    allowed_actions = Admin::Action.actions_for_controller params[:controller_name]
    current_action =  if params[:menu_item_id]
      current_item=Admin::MenuItem.find_by_id(params[:menu_item_id])
      (current_item && current_item.menuable.is_a?(Admin::Action)) ? current_item.menuable.action : nil
    else
      'list'
    end
